...
// Pointer to the current parser as a Julia function
jl_value_t *jl_parser_func = NULL;

JL_DLLEXPORT void jl_set_parser(jl_value_t *parser_func)
{
    jl_parser_func = parser_func;
}
                     ...
                     size_t offset, jl_value_t *options)
{
    ...
    args[0] = jl_parser_func;
    args[1] = jl_box_uint8pointer((uint8_t*)text);
    args[2] = jl_box_ulong(text_len);
    args[3] = filename;
    args[4] = jl_box_ulong(offset);
    args[5] = options;
    jl_value_t *result = jl_apply(args, 6);
    args[0] = result; // root during error handling
